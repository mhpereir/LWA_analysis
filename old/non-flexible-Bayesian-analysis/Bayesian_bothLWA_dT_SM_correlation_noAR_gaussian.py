...
OUTPUT_POSTERIORS_PATH = os.path.join(config.OUTPUT_PATH, "pymc_fits_noAR_bothLWA_gaussian")
os.makedirs(OUTPUT_POSTERIORS_PATH, exist_ok=True)

OUTPUT_PLOTS_PATH = os.path.join(config.OUTPUT_PATH, "plots/bayesian_bothLWA_dt_sm_correlation_noAR_gaussian")
...
def run_analysis(REGION: str, SEASON: str, ZG_COORD: int):
    ...
    for m in members_to_run:
        ...
        for model_name in model_variations:
            ...
            print(az.summary(idata_model_m, var_names=["b0","b1","b2","b3","sigma"]))
            # az.to_netcdf(
            #     idata_model_m,
            #     os.path.join(
